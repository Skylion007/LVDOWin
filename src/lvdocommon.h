...
static inline unsigned char prevent_byte_overflow(int x) {
    if(x > 255) {
        fprintf(stderr, "lvdo: [warning] clipping %d to 255\n", x);
        return 255;
    }
    if(x < 0) {
        fprintf(stderr,"lvdo: [warning] clipping %d to 0\n", x);
        return 0;
    }
    return x;
}
...
static inline unsigned char prevent_yuv_overflow(int x) {
    if(x > 235) {
        fprintf(stderr,"lvdo: [warning] clipping %d to 235\n", x);
        return 235;
    }
    if(x < 16) {
        fprintf(stderr,"lvdo: [warning] clipping %d to 16\n", x);
        return 16;
    }
    return x;
}
...
static inline void print_block(const unsigned int *block, unsigned int size) {
    ...
    for(i = 0; i < size; i++) {
        for(j = 0; j < size; j++)
            fprintf(stderr,"%d ", block[i*size+j]);
        fprintf(stderr,";\n");
    }
    fprintf(stderr,"\n");
}
...
static inline void print_block_double(const double *block, unsigned int size) {
    ...
    for(i = 0; i < size; i++) {
        for(j = 0; j < size; j++)
            fprintf(stderr,"%lf ", block[i*size+j]);
        fprintf(stderr,";\n");
    }
    fprintf(stderr,"\n");
}
